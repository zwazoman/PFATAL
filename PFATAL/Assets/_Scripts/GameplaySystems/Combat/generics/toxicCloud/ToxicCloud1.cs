...
public class ToxicCloud1 : NetworkBehaviour
{
    ...
    void ApplyDamage()
    {
        ...
        for (int i = 0; i < count; i++)
        {
            if (buffer[i].TryGetComponent(out DamageableObject hit))
            {
                ...
                TriggerClientEffectRPC(hit.OwnerClientId);
            }
        }
    }

    [Rpc(SendTo.SpecifiedInParams)]
    void TriggerClientEffectRPC(ulong clientId, RpcParams rpcParams = default)
    {
        // côté client seulement
        // effet écran vert | blur | vignette | distorsion
    }
}